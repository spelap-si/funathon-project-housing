...
df_sampled = df.sample(frac=0.1, axis=0, random_state= RANDOM_STATE)
...
3.  (*Optional*) Using the *Out-of-Bag (OOB) error*, try to plot the convergence of the OOB error as a function of the number of trees
    ...
    -   check if you have no type problems with data (for example with `Timestamp` column)
...
# --- Sous-échantillonnage stratifié sur la cible ---
    ...
    bins = np.unique(np.quantile(y_arr, np.linspace(0, 1, 100)))
    strata = np.digitize(y_arr, bins[1:-1])

    rng = np.random.default_rng(rf_params.get("random_state"))
    idx = np.concatenate([
        rng.choice(
            np.where(strata == s)[0],
            size=max(1, round((strata == s).sum() * subsample)),
            replace=False,
        )
        for s in np.unique(strata)
    ])

    X_sub = X_train[idx] if isinstance(X_train, np.ndarray) else X_train.iloc[idx]
    y_sub = y_arr[idx]

    # on traite le Timestamp présent dans les features
    X_sub["datemut"] = date_to_days(X_sub["datemut"], pd.Timestamp('2010-01-01 00:00'))
    X_sub = _encode_for_rf(X_sub)

    print(f"Sous-échantillon : {len(idx):,} / {len(y_arr):,} obs "
          f"({100 * len(idx) / len(y_arr):.1f}%)")
